...
function main() {
  ...
  assert(
    'only existing Supabase cookie chunks and their base are expired',
    expired.length === 3 &&
      expired.some((item) => item.name === 'sb-xxxx-auth-token') &&
      expired.some((item) => item.name === 'sb-xxxx-auth-token.0') &&
      expired.some((item) => item.name === 'sb-xxxx-auth-token.1') &&
  ...
  assert(
    'header size estimator is positive',
    estimateCookieHeaderBytes(expired) > 0,
  );
