    ...
    auth: {
      admin: {
        ...
        updateUserById: async () => {
          ...
          return {
            error: options.confirmError
              ? { message: options.confirmError }
              : null,
          };
        },
        generateLink: async () => {
...
async function main() {
...

  {
    ...
    assert(
      'missing configuration is controlled',
      result.ok === false && result.stage === 'configuration',
    );
    if (!result.ok) {
      assert(
        'no raw provider error',
        result.message === 'Unable to start a session. Please try again.',
      );
    }
  }

  {
    ...
    assert(
      'missing authUserId rejected',
      !result.ok && result.stage === 'auth_user_validation',
    );
  }
...

  {
    ...
    assert(
      'updateUserById API error controlled',
      !result.ok && result.stage === 'confirm_email',
    );
  }

  {
    ...
    assert(
      'updateUserById throw controlled',
      !result.ok && result.stage === 'confirm_email',
    );
  }

  {
    ...
    assert(
      'generateLink API error controlled',
      !result.ok && result.stage === 'generate_link',
    );
  }
...

  {
    ...
    assert(
      'verifyOtp API error controlled',
      !result.ok && result.stage === 'verify_magiclink',
    );
  }
...

  {
    ...
    assert(
      'returned user mismatch rejected',
      !result.ok && result.stage === 'session_validation',
    );
  }

  {
    const writes: Array<{ name: string }> = [];
    const browserCookieNames: string[] = [];
    const result = await createSessionForEmail(
      ...
      [{ name: 'sb-takatak-auth-token.0', value: 'stale' }],
      {
        getEnv: () => env,
        getAdmin: () => mockAdmin({ user: { id: AUTH_ID, email: EMAIL } }),
        createBrowserClient: (url, anon, cookieWrites, existingCookies) => {
          browserCookieNames.push(
            ...existingCookies.map((cookie) => cookie.name),
          );
    ...
    assert(
      'stale Supabase auth cookies are hidden from the verification client',
      !browserCookieNames.some((name) => name.includes('-auth-token')),
    );
    if (result.ok) {
      ...
      assert(
        ...
        result.cookies.some(
          (cookie) => cookie.name.endsWith('.0') && cookie.value === '',
        ),
      );
      assert(
        ...
        result.cookies.some(
          (cookie) =>
            cookie.name === 'takatak_active_client' && cookie.value === '',
        ) &&
          result.cookies.some(
            (cookie) =>
              cookie.name === 'takatak_active_brand' && cookie.value === '',
          ),
      );
      assert(
