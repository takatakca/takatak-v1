...
const cases: { name: string; input: TenantAccessInput; expect: (a: ReturnType<typeof computeTenantAccess>) => boolean }[] = [
  ...
  { name: "viewer, two memberships, none requested → first workspace", input: input({ profile: P("viewer"), memberships: M("c1", "c2") }), expect: (a) => a.mode === "client_scoped" && a.activeClientId === "c1" },
  ...
  { name: "TAMPERED cookie: ignore it and use a valid membership", input: input({ profile: P("viewer"), memberships: M("c1"), requestedClientId: "cEvil" }), expect: (a) => a.mode === "client_scoped" && a.activeClientId === "c1" },
  ...
  { name: "owner with a workspace, no cookie → client_scoped", input: input({ profile: P("owner"), memberships: M("c1") }), expect: (a) => a.mode === "client_scoped" && a.activeClientId === "c1" },
