...
function main() {
  ...
  check(
    ...
    'Google OAuth is billed as youtube for Social plan enforcement',
  );

  check(
    'Essential can connect Facebook',
    evaluateSocialNetworkConnect({
      provider: 'meta',
      entitlements: essential,
      connectedXCount: 0,
    }).allowed,
    'Essential includes Facebook',
  );

  check(
    'Essential can connect Google / YouTube',
    evaluateSocialNetworkConnect({
      provider: 'google',
      entitlements: essential,
      connectedXCount: 0,
    }).allowed,
    'YouTube is on Essential',
  );

  check(
    'Essential cannot connect LinkedIn',
    !evaluateSocialNetworkConnect({
      provider: 'linkedin',
      entitlements: essential,
      connectedXCount: 0,
  ...
  check(
    'Essential allows 20 posts then blocks',
    evaluateSchedulePost({ entitlements: essential, usedThisMonth: 19 })
      .allowed &&
      !evaluateSchedulePost({ entitlements: essential, usedThisMonth: 20 })
        .allowed,
  ...
  const cutoff = analyticsHistoryCutoffDate(
    30,
    new Date('2026-08-29T12:00:00.000Z'),
  );
  check(
    'Essential analytics window is 30 days',
    cutoff === '2026-07-31',
  ...
  check(
    'last_90 clips to 30 days on Essential',
    clipped.clipped &&
  ...
  check(
    'Essential competitor cap is 5',
    evaluateAddCompetitor({ entitlements: essential, activeCount: 4 })
      .allowed &&
      !evaluateAddCompetitor({ entitlements: essential, activeCount: 5 })
        .allowed,
    'the 6th competitor on Essential must be refused',
  );
  ...
  check(
    ...
    'unsubscribed, suspended, incomplete, and paused workspaces stay locked',
  );

  check(
    'Essential locks teams, roles, approvals, API, reports',
    !evaluateTeamInvite(essential).allowed &&
      !evaluateCustomRoles(essential).allowed &&
      !evaluateApprovals(essential).allowed &&
      !evaluateApiAccess(essential).allowed &&
      !evaluateReports(essential).allowed,
    'Essential is a single-user plan',
  );
