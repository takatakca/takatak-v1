...
import { THREADS_OAUTH_CALLBACK_PATH } from "@/lib/social/providers/threads-oauth";
import { exchangeThreadsCodeForStoredCredential } from "@/lib/social/providers/threads-token";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

const HANDOFF_COOKIE = "takatak_th_oauth_handoff";
const HANDOFF_PATH = "/api/social/callback/threads/handoff";
...
async function finishCallback(
  ...
  const result = await processDirectAccountOAuthCallback({
    ...
    rawQuery,
    profileId,
    exchangeCode: ({ code }) => exchangeThreadsCodeForStoredCredential({ code }),
  });
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  logSocialOAuthEvent("threads-oauth-callback", {
    stage: "ingress",
  ...
  try {
    ...
    response.cookies.set(
      ...
      directOAuthHandoffCookieOptions({
        origin,
        callbackPath: THREADS_OAUTH_CALLBACK_PATH,
      }),
    );
    return response;
  } catch {
