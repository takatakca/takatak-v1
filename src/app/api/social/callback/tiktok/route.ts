...
import { TIKTOK_OAUTH_CALLBACK_PATH } from "@/lib/social/providers/tiktok-oauth";
import { exchangeTikTokCodeForStoredCredential } from "@/lib/social/providers/tiktok-token";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

const HANDOFF_COOKIE = "takatak_tt_oauth_handoff";
const HANDOFF_PATH = "/api/social/callback/tiktok/handoff";
...
async function finishCallback(
  ...
  const result = await processDirectAccountOAuthCallback({
    ...
    rawQuery,
    profileId,
    exchangeCode: ({ code }) => exchangeTikTokCodeForStoredCredential({ code }),
  });
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  logSocialOAuthEvent("tiktok-oauth-callback", {
    stage: "ingress",
