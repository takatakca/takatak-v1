...
import { X_OAUTH_CALLBACK_PATH } from "@/lib/social/providers/x-oauth";
import { exchangeXCodeForStoredCredential } from "@/lib/social/providers/x-token";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

const HANDOFF_COOKIE = "takatak_x_oauth_handoff";
const HANDOFF_PATH = "/api/social/callback/x/handoff";
...
async function finishCallback(
  ...
  const result = await processDirectAccountOAuthCallback({
    ...
    exchangeCode: ({ code, codeVerifier }) =>
      exchangeXCodeForStoredCredential({ code, codeVerifier }),
  });
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  logSocialOAuthEvent("x-oauth-callback", {
    stage: "ingress",
  ...
  try {
    ...
    response.cookies.set(
      ...
      directOAuthHandoffCookieOptions({
        origin,
        callbackPath: X_OAUTH_CALLBACK_PATH,
      }),
    );
    return response;
  } catch {
