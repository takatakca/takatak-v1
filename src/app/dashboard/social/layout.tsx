...
export default async function SocialLayout({
  ...
  const providers: ManageConnectionsProvider[] =
    listSocialProviderReadiness().map((readiness) => {
      ...
      return {
        ...
        connectable: readiness.connectable && planAllowsConnect,
        configured: readiness.configured,
        supportsMultipleAccounts: readiness.supportsMultipleAccounts,
  ...
  if (connectionRecords && connectionRecords !== 'unavailable') {
    ...
    const connectedAccounts = records.flatMap((connection) =>
      connection.accounts
        .filter((account) => {
          if (
    ...
    const platforms = [
      ...new Set(connectedAccounts.map((account) => account.platform)),
    ];
    accounts = platforms
      .map((platform) =>
        pickConnectedPlatformAccount(connectedAccounts, platform),
      )
      .filter(
  ...
  return (
    <SocialWorkspaceShell
      data={{
        activeBrandId: brandContext.activeBrandId,

        activeBrandName: brandContext.activeBrandName,

        activeBrandDisplayLabel: brandContext.activeBrandDisplayLabel,

        activeBrandDisplayImageUrl: brandContext.activeBrandDisplayImageUrl,

        brands: brandContext.availableBrands,

        accounts,
        providers,
        connections,

        canManageSocialAccounts: hasEffectivePermission(
          access,
        ...
        brandAllowance: billing?.entitlements.brandAllowance ?? 0,
