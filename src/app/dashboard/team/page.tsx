...
export default async function TeamPage() {
  ...
  const canInvite = hasEffectivePermission(
    access,
    "invite_users",
  );
  ...
  const canSuspendUsers = hasEffectivePermission(
    access,
    "suspend_users",
  );

  const canDeleteUsers = hasEffectivePermission(
    access,
    "delete_users",
  );
  ...
  const allowedInvitationRoles = ROLE_OPTIONS.filter(
    (candidateRole) => {
      ...
      <header className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-600">
            Manage access to {data.clientName}.
          </p>
        </div>
