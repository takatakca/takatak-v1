...
/** Honest FLEXS preparation panel — nothing is connected. */
export function FlexsPrepPanel() {
  return (
    <Card>
      <CardHeader
        title="FLEXS Preparation"
        subtitle="FLEXS becomes the lead generation engine in a future phase. Nothing is connected yet."
        action={<Badge tone="warning">Not connected</Badge>}
      />
      <CardBody className="space-y-3">
        <div className="flex items-center gap-2 text-xs text-slate-500">
          <PlugZap className="h-4 w-4 text-slate-400" />
          Requires FLEXS_API_KEY (env var — name only, no value stored in code).
        </div>
        ...
        <div className="flex gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2">
          ...
          <p className="text-[11px] leading-relaxed text-amber-800">
            No FLEXS API calls, lead imports, CRM sync, email/SMS outreach, or automation workers are active in Phase 12.
          </p>
        </div>
      </CardBody>
    </Card>
