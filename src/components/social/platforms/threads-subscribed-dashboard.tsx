...
type ThreadsMetric = number | null;

export type ThreadsCommunityData = {
  followers: ThreadsMetric;
  following: ThreadsMetric;
  totalContent: ThreadsMetric;
  dailyFollowers: ThreadsMetric;
  followersPerPost: ThreadsMetric;
  dailyPosts: ThreadsMetric;
  postsPerWeek: ThreadsMetric;
  acquired: ThreadsMetric;
  lost: ThreadsMetric;
...
const EMPTY_DATA: ThreadsCommunityData = {
  followers: null,
  following: null,
  totalContent: null,
  dailyFollowers: null,
...
const ADVANCED_ACCOUNT_CONTENT: Record<
  Exclude<ThreadsAccountView, "general">,
  {
    metric: string;
    divideBy: string;
    title: string;
...
function AdvancedAccountContent({
  ...
  view: Exclude<ThreadsAccountView, "general">;
}) {
      ...
      <div className="mt-8 flex flex-col gap-5 sm:flex-row">
        ...
        <label className="block w-full max-w-[325px] text-sm text-[#81909D]">
          ...
          <select
            ...
            onChange={() => undefined}
            aria-label="Divide Threads account analytics"
            className="h-[58px] w-full rounded-[9px] border border-[#9EAFBC] bg-white px-4 text-[16px] text-[#81909D] outline-none"
          >
...
function AccountSection({
  ...
  const [accountView, setAccountView] =
    useState<ThreadsAccountView>("general");
  ...
  return (
    <div className="space-y-7">
      ...
      <section className="overflow-hidden rounded-[14px] border border-[#E8EAED] bg-white px-5 pb-2 pt-6 shadow-[0_2px_12px_rgba(31,41,55,0.04)] sm:px-7">
        <div
          role="tablist"
          aria-label="Threads Account analytics"
          ...
          {ACCOUNT_VIEWS.map((item) => {
            ...
            return (
              <button
                ...
                aria-controls={`threads-account-${item.key}`}
                id={`threads-account-tab-${item.key}`}
        ...
        <div
          id={`threads-account-${accountView}`}
          role="tabpanel"
          aria-labelledby={`threads-account-tab-${accountView}`}
        >
          {accountView === "general" ? (
            <>
              <label className="mt-8 block max-w-[260px] text-sm text-[#8B97A2]">
                ...
                <select
                  ...
                  aria-label="Threads account metric"
                >
    ...
    <article className="min-w-0">
      <div className="flex items-center justify-between gap-4">
        ...
        <button
          ...
          aria-controls={`threads-posts-${title.toLowerCase()}-view`}
          onClick={() => {
      ...
      <div
        id={`threads-posts-${title.toLowerCase()}-view`}
...
function downloadEmptyThreadsCsv(
  filename: string,
  columns: string[],
) {
...
function ThreadsReelsSection({
  ...
  /*
   * A confirmed empty Threads account supports a real zero.
...
function ThreadsStoriesEvolution({
  impressions,
  averageReach,
  stories,
  start,
...
function ThreadsStoriesSection({
  data,
  start,
  end,
}: {
...
function ThreadsCollabsEvolution({
  value,
  start,
  end,
}: {
...
function ThreadsCollabsSection({
  data,
  start,
  end,
}: {
   ...
   * Threads synchronization contract provides them.
   */
  const confirmedEmpty = data.totalContent === 0;
  const zeroWhenConfirmedEmpty = confirmedEmpty
    ? 0
...
function ThreadsCompetitorsSection() {
...
export function ThreadsSubscribedDashboard({
  data = EMPTY_DATA,
  accountName,
  ...
  const [tab, setTab] = useState<ThreadsTab>("community");
  const [range, setRange] = useState<number>(30);
  const [compareEnabled, setCompareEnabled] = useState(false);

  function scrollToSection(section: ThreadsTab) {
    setTab(section);
  ...
  useEffect(() => {
    const sections = TABS.map((item) =>
      document.getElementById(`threads-${item.key}`),
    ).filter(
  ...
  return (
    <div className="-mx-1 space-y-5 bg-[#FCFCFD] px-4 pb-24 pt-1 sm:px-5 lg:px-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-[#5F6770]">
              Selected {start} → {end}
              {hasConfirmedData
                ? " · Confirmed account-specific Threads data"
                : " · Waiting for synchronized Threads data"}
            </p>

            {!hasConfirmedData ? (
              <p className="mt-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-950">
                No synchronized Threads data overlaps this range.
              </p>
            ) : null}
          </div>
        </div>
      ...
      <div className="sticky top-[66px] z-20 flex flex-col gap-3 border-b border-[#E1E4E7] bg-[#FCFCFD]/95 pb-3 pt-3 backdrop-blur-sm lg:flex-row lg:items-end lg:justify-between">
        <nav
          aria-label="Threads analytics sections"
          ...
          <label className="flex flex-col gap-1.5 text-xs font-medium text-[#6B7280]">
            ...
            <span className="flex h-11 items-center gap-2 rounded-[10px] border border-[#D7DBE0] bg-white px-3 text-sm shadow-sm">
              ...
              <select
                ...
                aria-label="Select Threads date range"
      ...
      <div className="lg:hidden">
        <label className="sr-only" htmlFor="threads-mobile-tab">
          Threads section
        </label>
        <select
          id="threads-mobile-tab"
          value={tab}
          onChange={(event) =>
            scrollToSection(
              event.target.value as ThreadsTab,
            )
          }
      ...
      <section
        id="threads-community"
        aria-labelledby="threads-community-heading"
        className="scroll-mt-[190px] space-y-5"
      >
        <h2
          id="threads-community-heading"
          className="sr-only"
        >
          Threads Community analytics
        </h2>
        ...
        <div className="rounded-[10px] border border-[#A8B4FF] bg-[#F0F1FF] px-4 py-3 text-[13px] leading-5 text-[#46516A]">
          ...
          Threads account is connected. TAKATAK preserves confirmed
      ...
      <section
        id="threads-account"
        aria-labelledby="threads-account-heading"
        className="scroll-mt-[190px]"
      >
        <h2
          id="threads-account-heading"
          className="sr-only"
        >
      ...
      <section
        id="threads-posts"
        aria-labelledby="threads-posts-heading"
        className="scroll-mt-[190px]"
      >
        <h2
          id="threads-posts-heading"
          className="sr-only"
        >
      ...
      <section
        id="threads-reels"
        aria-labelledby="threads-reels-heading"
        className="scroll-mt-[190px]"
      >
        <h2
          id="threads-reels-heading"
          className="sr-only"
        >
          Threads Reels analytics
        </h2>
      ...
      <section
        id="threads-stories"
        aria-labelledby="threads-stories-heading"
        className="scroll-mt-[190px]"
      >
        <h2
          id="threads-stories-heading"
          className="sr-only"
        >
          Threads Stories analytics
        </h2>

        <ThreadsStoriesSection
          data={data}
          start={start}
          end={end}
        />
      ...
      <section
        id="threads-collabs"
        aria-labelledby="threads-collabs-heading"
        className="scroll-mt-[190px]"
      >
        <h2
          id="threads-collabs-heading"
          className="sr-only"
        >
          Threads Collabs analytics
        </h2>

        <ThreadsCollabsSection
          data={data}
          start={start}
          end={end}
        />
      ...
      <section
        id="threads-competitors"
        aria-labelledby="threads-competitors-heading"
        className="scroll-mt-[190px]"
      >
        <h2
          id="threads-competitors-heading"
          className="sr-only"
        >
          Threads Competitors analytics
        </h2>

        <ThreadsCompetitorsSection />
      </section>
