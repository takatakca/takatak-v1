...
export function TikTokConnectPage({
  ...
  async function startOAuth() {
    ...
    if (!activeBrandId) {
      setError("Choose an active brand before connecting TikTok.");
      return;
    }
    ...
    try {
      const response = await fetch("/api/social/connections/start", {
        ...
        body: JSON.stringify({
          ...
          returnPath: withSocialPreview(
            "/dashboard/social/tiktok?connections=open",
            searchParams,
          ),
        }),
      });
      ...
      if (!response.ok || !result.ok || !authorizationUrl) {
        setError(result.message ?? "TikTok authorization could not be started.");
        setBusy(false);
        return;
      }
      ...
      setError("TikTok authorization could not be started.");
      setBusy(false);
    }
  }
  ...
  if (resolutionIssue === "ambiguous") {
    return (
      <div className="rounded-[14px] border border-amber-200 bg-amber-50 px-5 py-6 text-sm text-amber-950">
        Multiple TikTok selections need attention. Open Manage connections and
        confirm a single TikTok account for this brand.
      </div>
    );
  }
      ...
      <div className="space-y-7 px-1 pb-16 pt-2">
        <header className="flex items-center gap-4">
          {profileImageUrl ? (
            ...
            <span className="flex h-14 w-14 items-center justify-center rounded-full bg-black text-lg font-semibold text-white">
              {(connectedLabel ?? "TikTok").slice(0, 1).toUpperCase()}
            </span>
          )}
          <div>
            <p className="text-sm font-medium text-slate-500">TikTok</p>
            <h1 className="text-[28px] font-semibold leading-tight text-[#20242A]">
              {connectedLabel ?? "TikTok account"}
            </h1>
          </div>
        </header>
        ...
        <section className="rounded-[18px] border border-slate-200 bg-white px-7 py-6">
          <h2 className="text-[18px] font-semibold text-slate-950">
            {isConnected ? "Account connected" : "No TikTok account connected"}
          </h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
            {isConnected
              ? "Account connected. Analytics will appear here after the next sync step."
              : "Your saved TikTok history remains stored. Reconnect the same account to restore only that account’s analytics."}
          </p>
          {!isConnected ? (
            <Link
        ...
        <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-2 max-w-xl text-[15px] leading-6 text-[#505761]">
              Sign in with TikTok to import a personal account. Business and Ads
      ...
      <section className="overflow-hidden rounded-[18px] border border-[#e1e4e7] bg-white">
        <div className="grid lg:grid-cols-3 lg:divide-x lg:divide-[#e1e4e7]">
          <EducationColumn
            ...
            description="See how your TikTok community grows after the account is connected."
            icon={MapPin}
            image="/social/summary/community-growth.webp"
            imageAlt="Audience growth preview"
          />
          <EducationColumn
            title="Analyze videos and views"
            description="Understand which TikTok videos drive views and engagement."
            icon={Eye}
            image="/social/summary/post-reach-bg.webp"
            imageAlt="Post performance preview"
          />
          <EducationColumn
            title="Independent TikTok login"
            description="Connect a TikTok personal account through Login Kit. Facebook is not required."
            icon={RefreshCw}
