  ...
  return (
    <section className="brand-dark relative overflow-hidden border-y border-border py-16 md:py-24">
      <div
        ...
        style={{ background: "radial-gradient(700px 300px at 80% 20%, color-mix(in oklab, var(--brand-accent-violet) 18%, transparent), transparent 60%)" }}
      />
      <div className="relative mx-auto max-w-7xl px-4">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div className="max-w-2xl">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-primary">Managed hosting</p>
            <h2 className="mt-3 text-3xl font-bold leading-tight text-foreground md:text-4xl">
              Fast, secure hosting from portfolio to ecommerce
            </h2>
            <p className="mt-3 text-base leading-7 text-muted-foreground">
              LiteSpeed-powered plans with SSL, backups, email, and TAKATAK-managed support paths. Change plans anytime.
            </p>
          </div>
          <Link
            to="/hosting"
          ...
          >
            View all hosting plans <ArrowRight size={14} />
          </Link>
        </div>

        <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {pricing.hosting.map((plan, i) => {
            const featured = plan.key === "silver";
            return (
              <div
                key={plan.key}
                className={
              ...
              >
                ...
                <div className="mt-4 flex items-baseline gap-1">
                  <span className="text-3xl font-bold tracking-tight text-foreground">{formatCAD(plan.amount)}</span>
                ...
                <Link
                  ...
                  style={featured ? { backgroundImage: "var(--gradient-hero)" } : undefined}
                >
                  Choose {plan.name}
                  <ArrowRight size={14} />
                </Link>
                <div className="mt-3 text-[10px] uppercase tracking-wider text-muted-foreground text-center">
                  Plan {i + 1} of {pricing.hosting.length}
                </div>
              </div>
            );
          })}
