...
export const SOCIAL_BILLING_HREF = '/dashboard/social/settings?tab=billing';
...
export function resolveSocialBillingBanner(
  ...
  if (input.cancelAtPeriodEnd && input.periodEndLabel) {
    return {
      ...
      body: `Paid access continues until ${input.periodEndLabel}. After that Social access is locked until another plan is purchased.`,
      actionLabel: 'View billing',
    };
  }
    ...
    },
    {
      ...
      unlock:
        entitlements.monthlyPostAllowance === null
          ? 'Unlimited / fair use'
          : entitlements.planCode === 'social_unsubscribed'
            ? 'Social Essential or higher'
            : `${entitlements.monthlyPostAllowance} on Social Essential · Starter unlocks unlimited`,
    },
    {
      id: 'analytics',
      ...
      unlock:
        entitlements.analyticsHistoryDays === null
          ...
          : entitlements.planCode === 'social_unsubscribed'
            ? 'Social Essential or higher'
            : `${entitlements.analyticsHistoryDays} days on Social Essential · Starter unlocks the full window`,
    },
    {
      id: 'reports',
      feature: 'Reports',
      included: entitlements.reports,
      unlock: entitlements.reports
        ? 'Included on this plan'
        : 'Starter or Advanced',
    },
    {
