...
export type SocialStripePlanChange =
  'checkout' | 'upgrade' | 'downgrade' | 'interval' | 'noop' | 'forbidden';
...
export function resolveSocialCheckoutLive(input: {
  ...
  return Boolean(
    input.secretKey.trim() && input.webhookSecret.trim() && input.hasPrice,
  );
}
...
/**
 * Determines how a workspace moves from its current billing state to a
 * self-serve paid plan. Checkout never unlocks access directly; the Stripe
 * webhook confirms payment and applies the paid subscription.
 */
export function resolveSocialStripePlanChange(input: {
  currentPlanCode: SocialPlanCode;
  targetPlanCode: string;
  hasStripeSubscription: boolean;
  currentAccess: 'paid' | 'blocked';
...
export function validateSocialStripeCheckoutInput(
  value: unknown,
): ValidationResult<{
  planCode: PaidCheckoutPlanCode;
  ...
  if (!isPaidCheckoutPlanCode(planCode)) {
    fieldErrors.planCode =
      planCode === 'social_custom'
        ...
        : 'Choose a Social Essential, Starter, or Advanced plan.';
  }
